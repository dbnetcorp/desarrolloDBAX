...
from   (select	codi_info, 
				...
				letr_dime
		from    xbrl_dime_defi 
		where   codi_dime like '%cl-cs%2013%'
		) tmp1) tmp
union
...
from   (select	c.codi_info	codi_info, 
		...
		from    xbrl_dime_defi d,
				xbrl_info_conc c
		where   d.codi_dime like '%cl-cs%2013%'
...
from    xbrl_dime_axis
where   codi_axis like '%cl-cs%2013%') tmp) tmp1
union
...
select	substring(codi_axis, charindex('#',codi_axis) + 1, 100) codi_axis,
		...
		tipo_memb
from    xbrl_dime_memb
where   codi_axis like '%cl-cs_cor_2013-01-31%') tmp) tmp1
